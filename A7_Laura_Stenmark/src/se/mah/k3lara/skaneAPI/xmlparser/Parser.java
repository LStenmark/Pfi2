	...
	public static Lines getStationResults(Station station){
		...
		if (xml!=null){
			...
			for (int i = 0; i < nl.getLength(); i++) { //Iterate all Line elements
				...
				//Get the value for that tag "Name"
				name = parser.getValue(e, "Name"); 
				...
				//Get the value for that tag "isTimingPoint"
				isTimingPoint = parser.getValue(e, "IsTimingPoint");
				...
				//Get the value for that tag "depDeviationAffect"
				depDeviationAffect = parser.getValue(e, "DepDeviationAffect");
				
				//Get the value for that tag "TrainNo"
				TrainNo = parser.getValue(e, "TrainNo");
				
				//Get the value for that tag "RumNo"
				RumNo = parser.getValue(e, "RunNo");
				
				//Then we got one Line lets create a line object and add it to Lines
				Line l = new Line();
				l.setName(name);
				l.setIsTimingPoint(isTimingPoint);
